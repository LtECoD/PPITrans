...
def build_data(proteins):
    ...
    label = np.array([types[_id] for _id in seq_id])
...
if __name__ == '__main__':
    ...
    for orga in organisms:
        train_proteins[orga] = load_proteins(os.path.join(args.protein_dir, "train"), orga)
        print(f"{orga} train set size: {sum([p.length for p in train_proteins[orga]])}")
        test_proteins[orga] = load_proteins(os.path.join(args.protein_dir, "test"), orga)
